...
def build_silver_order_items() -> None:
    try:
        ...
        df = df.rename(columns={
            ...
            "node.originalUnitPriceSet.shopMoney.amount": "unit_price",     # NEW: list price per unit
            "node.discountedTotalSet.shopMoney.amount": "line_revenue",     # NEW: actual $ for the line (net of discounts)
            "node.product.id": "product_id",
            "node.product.productType": "product_type",
        })

        df["quantity"] = pd.to_numeric(df["quantity"], errors="coerce")
        df["unit_price"] = pd.to_numeric(df["unit_price"], errors="coerce")        # NEW
        df["line_revenue"] = pd.to_numeric(df["line_revenue"], errors="coerce")    # NEW
        df["order_id"] = df["order_id"].str.split("/").str[-1]        # strip gid prefix
        df["product_id"] = df["product_id"].str.split("/").str[-1]    # strip gid prefix (null -> NaN)

        df = df[["order_id", "product_id", "title", "quantity",
                 "unit_price", "line_revenue", "product_type"]]       # NEW cols added
        ...
        log_message("Data quality check", stage="silver", level="INFO",
                    ...
                    null_revenue=int(df["line_revenue"].isna().sum()))    # NEW check
